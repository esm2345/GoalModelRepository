# Model Metadata: PatientAndProvider_remoteHealthcare

| Label | Description |
| :--- | :--- |
| **title** | PatientAndProvider_remoteHealthcare |
| **source_paper** | Sensitivity Analysis of Conflicting Goals in the i* Goal Model |
| **DOI** |10.1093/comjnl/bxaa189 |
| **authors** | S. Sumesh, A. Krishna|
| **year** | 2022|
| **domain** | Telemedicine |
| **model_origin** |  reconstructed-from-image|
| **original_notation_and_extension** | iStar 1.0 |
| **reconstruction_notes** | • **Contribution Link Consolidation**: The original model distinguishes between more link types (*Help* vs. *Some+* and *Hurt* vs. *Some-*). Due to tool syntax limitations during reconstruction, these were simplified and modeled uniformly as standard **Help** (for positive contributions) and **Hurt** (for negative contributions) links respectively. |
| **interpretation_guidance** | / |
| **supplementary_material** | /|

---

## Folder Contents
* **`model.txt`**: The new reconstructed model file, compatible with the PiStar tool
* **`model.png`**: A high-resolution export of the model for quick viewing
