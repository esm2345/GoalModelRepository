# Model Metadata: Microgrid_EnergyManagement

| Label | Description |
| :--- | :--- |
| **title** | Microgrid_EnergyManagement|
| **source_paper** | Multi-agent based coalition formation of prosumers in microgrids using the * goal modelling|
| **DOI** |10.3233/KES-230902 |
| **authors** |S. Sumesh, A. Krishna |
| **year** | 2023|
| **domain** | Energy Management |
| **model_origin** |  reconstructed-from-image |
| **original_notation_and_extension** | iStar 1.0|
| **reconstruction_notes** | • **Link Mapping**: refactored *Some+* and *Some-* links into *Help* and *Hurt* links to fit the modeling tool's constraints.<br>• **Dependency Fixes**: Refactored cross-actor *Help links* into softgoal dependencies, anchoring them (if needed) to the lowest-level leaf nodes to keep the syntax happy. |
| **interpretation_guidance** | / |
| **supplementary_material** |/|

---

## Folder Contents
* **`model.txt`**: The new model file, compatible with the PiStar tool
* **`model.png`**: A high-resolution export of the model for quick viewing
